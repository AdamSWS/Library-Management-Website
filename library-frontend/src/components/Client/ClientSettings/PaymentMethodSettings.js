...
function PaymentMethodSettings({ email }) {
    ...
    const fetchCards = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/client/${email}/cards`);
    ...
    const handleDelete = async (cardNumber) => {
        try {
            const response = await axios.delete(`http://localhost:4000/client/${email}/cards/${cardNumber}`);
            if (response.data.success) {
...
const handleUpdate = async () => {
    ...
    return (
        <div className="bg-white p-4 rounded-lg shadow-lg">
            ...
            />
            <button onClick={handleAdd} className="btn btn-primary mb-3">Add New Method</button>
            {selectedCard && (
                <div>
                    <h3>Update Method</h3>
                    <input 
                        ...
                        className="form-control mb-3" 
                    />
                    <button onClick={handleUpdate} className="btn btn-success mb-3">Update Method</button>
                </div>
            )}
                    ...
                    {cards.map(card => (
                        <tr key={card.card_number} onClick={() => selectCard(card)}>
                            ...
                            <td className="border px-4 py-2">
                                <button onClick={() => handleDelete(card.card_number)} className="btn btn-danger">Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
