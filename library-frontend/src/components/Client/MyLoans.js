...
export default function MyLoans() {
    ...
    const handlePayFee = () => {
        if (!selectedLoanId) return;
        const updatedLoans = loans.map(loan => loan.id === selectedLoanId ? { ...loan, fee: 0 } : loan);
        setLoans(updatedLoans);
    };
            ...
            <div className="card shadow-lg h-100">
                ...
                <div className="card-body">
                    <div className="mb-3">
                        ...
                        <button onClick={handlePayFee} disabled={!selectedLoanId || loans.find(loan => loan.id === selectedLoanId)?.fee === 0} className="btn btn-danger">Pay Fee for Selected</button>
                    </div>
